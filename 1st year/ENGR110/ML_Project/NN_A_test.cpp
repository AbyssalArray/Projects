...
void save_vector_to_file_3d(std::vector<double> v, std::vector<double> v2, std::vector<double> v3){
	std::string file_name = "graph-50N-6k";
	std::ofstream ofs;
	...
	if (ofs.is_open()){
		...
		for (uint i = 0 ; i < v.size(); i++){
			ofs<<v.at(i)<<" "<<v2.at(i)<<" "<<v3.at(i)<<std::endl;
			count++;
		}
		ofs.close();
	}
...
void NN::load_neurons(std::string file_name){
    std::ifstream ifs;
    std::cout<<"Opening neural net into "<<file_name<<" ...";
...
int main(int argc, char **argv){
	...
	for (int i = 20; i > 0; i -= 1) {
		for (int j = 20; j > 0; j -= 1) {
			...
			std::string wb_file_name = "6k/N"+ neuronCount +"wbls" + std::to_string(i)+ "ds" + std::to_string(j);
			
			//std::cout<<" main() arguments are:\n"<<argc<<std::endl;
			int argcc = argc; 
			while(argcc--)
			{
				std::string h = "argc=" + std::to_string(argcc) + "    " + argv[argcc];
			}
			if (argc == 2){
				wb_file_name = argv[argc-1];
			} else {
				//std::cout<<wb_file_name;
			}
			...
			std::cout<<j<<" Rate="<<100 * (double)n_correct/test_set.data_points.size()<< "%			"<<std::endl;
			lr.push_back((double)i/10);
			dev.push_back((double)j/10);
			acc.push_back(100 * (double)n_correct/test_set.data_points.size());
			//neuralNet.DisplayResults(test_set);
		}
	}
	 save_vector_to_file_3d(lr, dev, acc);
}
